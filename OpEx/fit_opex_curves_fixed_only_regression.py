...
###### Scaling. This converts the OpEx ($/kW-yr) from the 15 MW 1 GW value into a $/kW-yr value for a 1GW farm on the respective turbine size.  Same scaling factors for fixed and floating.
...
### Inputs for fit
csv_ifile = 'opex_15MW_' + substruct + '_USD_per_kWyr.csv' #"opex_15MW_fixed_USD_per_kWyr.csv" # output of WOMBAT
...
for turbine in [15]:
    ...
    #print('Fixed OpEx ($/kW) for ' + str(turbine) + ' MW at 1 GW farm = ' + str(m[0]) + '*Distance*swh + ' + str(intercept))
    ...
    #ax.set_title('Fixed OpEx ($/kW-yr) for 1 GW farm using ' + str(turbine) + 'MW')
    ...
    ax.set_xlabel('Distance from shore [km]', labelpad=10)
    ax.set_ylabel('Wave height [m]', labelpad = 10)
    ax.set_zlabel('OpEx ($/kW-yr)', labelpad=10)
    ...
    #for label in ax.zaxis.get_ticklabels()[::2]:
     #   label.set_visible(False)
    ax.set_zlim([55,80])
    plt.savefig('figures/modelled_opex_func_of_dist_and_swh_' + str(turbine) + '_' + str(substruct) + '.png', bbox_inches='tight')
    ...
    # ax.set_title('% difference of Modelled and Actual OpEx (fixed)')
    # ax.set_xlabel('Distance from shore [km]')
    # ax.set_ylabel('Wave height [m]')
    # ax.set_zlabel('% Difference')
    # #ax.cbar()
    # plt.savefig('percent_diff_btwn_modelled_and_actual' + str(turbine) + '_fixed.png', bbox_inches='tight')
    # #ax.invert_xaxis()
    ...
    # ax.set_xlabel('Distance from shore [km]')
    # ax.set_ylabel('Wave height [m]')
    # plt.savefig('percent_diff_btwn_modelled_and_actual_2d' + str(turbine) + '_fixed.png', bbox_inches='tight')
