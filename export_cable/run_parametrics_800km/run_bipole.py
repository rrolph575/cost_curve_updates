...
#     weather.to_pickle('output/weather_' + custom_config[8:-5] + '.pkl')
    ...
    cable_range = ['HVDC_2500mm_525kV'] # 'XLPE_1000m_220kV_dynamic'
    parameters = {
    ...
    dist = np.concatenate([distance_range])
    oss_install_dc = 90 * np.ones(39)
    oss_install_ac = 3 * np.ones(39)
    cable_install_dc = 2.7 / 1.61 * np.ones(39)
    cable_install_ac = 0.609* 3 * np.ones(39)
    
    oss_install = np.concatenate([oss_install_dc])
    cable_install = np.concatenate([cable_install_dc])
    ...
    for c in cable_range:
        ...
        cable_results[c] = parametric.results.loc[parametric.results['export_system_design.cables'] == c]['total_cost'].values
        ax.plot(distance_range, cable_results[c], 'x', label=l)
...
# #     ax.plot(x, y, '.-', label='New cost curve')
...
# #     ax.plot(x, orca_float_export, ':', label='ORCA cost equation')
    ...
    DNV_HVDC = 1000 + 4.38 * x
    
#     ax.plot(x, DNV_HVAC, label='DNV (HVAC)')
    ax.plot(x, DNV_HVDC, label='DNV (HVDC bipole)')
    
    ax.legend()
    ax.set_xlabel('Distance from landfall, km')
    ax.set_ylabel('Export system cost, $ millions')
